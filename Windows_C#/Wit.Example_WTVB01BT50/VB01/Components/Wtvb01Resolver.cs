...
namespace Wit.SDK.Modular.Sensor.Modular.ProtocolResolver.Roles
{
     ...
     * 蓝牙5.0协议解析器
     */
    public class Wtvb01Resolver : IProtocolResolver
    {
        ...
        public override void OnReceiveData(DeviceModel deviceModel, byte[] data)
        {
            lock (lockobj)
            {
                ...
                while (ActiveByteDataBuffer.Count >= 28)
                {

                    ActiveByteTemp = ActiveByteDataBuffer.GetRange(0, 28).ToArray();

                    //必须是55 61的数据包
                    if (ActiveByteTemp[0] == 0x55 && ActiveByteTemp[1] == 0x61)
                    {
                        short[] Pack = new short[13];
                        ...
                        Pack[9] = BitConverter.ToInt16(ActiveByteTemp, 20);
                        Pack[10] = BitConverter.ToInt16(ActiveByteTemp, 22);
                        Pack[11] = BitConverter.ToInt16(ActiveByteTemp, 24);
                        Pack[12] = BitConverter.ToInt16(ActiveByteTemp, 26);
                        ...
                        deviceModel.SetDeviceData(new ShortKey(Identify + "_9"), Pack[9]);
                        deviceModel.SetDeviceData(new ShortKey(Identify + "_10"), Pack[10]);
                        deviceModel.SetDeviceData(new ShortKey(Identify + "_11"), Pack[11]);
                        deviceModel.SetDeviceData(new ShortKey(Identify + "_12"), Pack[12]);
                        ActiveByteDataBuffer.RemoveRange(0, 28);
                    }
                    else
                    {
                        // 不是就移除一个
