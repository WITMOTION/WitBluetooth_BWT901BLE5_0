...
public class WitBleResolver : IProtocolResolver
{
    /// <summary>
    /// 主动接收的byte缓存
    /// Actively received data cache
    /// </summary>
    ...
    /// <summary>
    /// 处理被动接收的数据
    /// Handling actively returned data
    /// </summary>
        ...
        lock (lockobj)
        {
            ...
            while (ActiveByteDataBuffer.Count >= 20)
            {
                ...
                if (ActiveByteTemp[0] == 0x55 && ActiveByteTemp[1] == 0x61)
                {
                    ...
                    deviceModel.SetDeviceData(Identify + "_0", Pack[0] + "");
                    deviceModel.SetDeviceData(Identify + "_1", Pack[1] + "");
                    deviceModel.SetDeviceData(Identify + "_2", Pack[2] + "");
                    deviceModel.SetDeviceData(Identify + "_3", Pack[3] + "");
                    deviceModel.SetDeviceData(Identify + "_4", Pack[4] + "");
                    deviceModel.SetDeviceData(Identify + "_5", Pack[5] + "");
                    deviceModel.SetDeviceData(Identify + "_6", Pack[6] + "");
                    deviceModel.SetDeviceData(Identify + "_7", Pack[7] + "");
                    deviceModel.SetDeviceData(Identify + "_8", Pack[8] + "");
                    ActiveByteDataBuffer.RemoveRange(0, 20);
                }
                else
                {
                    // 不是就移除一个 Must be a packet of 55 61
                    ActiveByteDataBuffer.RemoveAt(0);
                }
        ...
        if (sendData != null && sendData.Length >= 5 && sendData[2] == 0x27 && returnData != null && returnData.Length >= 11)
        {
            ...
            if (returnData != null && returnData.Length == 20)
            {
                ...
                if (readReg == rtnReg)
                {
                    ...
                    deviceModel.SetDeviceData((readReg + 0).ToString("X2"), Pack[0] + "");
                    deviceModel.SetDeviceData((readReg + 1).ToString("X2"), Pack[1] + "");
                    deviceModel.SetDeviceData((readReg + 2).ToString("X2"), Pack[2] + "");
                    deviceModel.SetDeviceData((readReg + 3).ToString("X2"), Pack[3] + "");
                }
            }
        }
    }
